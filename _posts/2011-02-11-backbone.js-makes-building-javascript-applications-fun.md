...
Backbone.js] describes itself as suppling structure to JavaScript-heavy 
...
There is some server-side code that replaces the Rails Show Exceptions page and syntax highlights snippet of source code for each line in the stack but
...
![Code Buddy](/examples/code_buddy/images/code_buddy_screenshot.png)

There's a lot going on here and if we had tried building this before discovering backbone.js we probably would have had mess
...
![Code Buddy views on the page](/examples/code_buddy/images/annotated_screenshot.png)
...
Finally, `tabName` is used to wrap this html in an `li` tag.  
...
![Showing Code for selected Addresss](/examples/code_buddy/images/code_buddy_screenshot_with_selection.png)
